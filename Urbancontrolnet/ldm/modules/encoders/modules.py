...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def forward(self, text):
        batch_encoding = self.tokenizer(text, truncation=True, max_length=74, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")
        tokens = batch_encoding["input_ids"].to(self.device)
        ...
        if text[0] !='':
            ...
            max_values = [ 150, 100, 38.74]   #1689.0, 1000.0,
...
class FrozenLongCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14", device="cuda", max_length=200,
                 freeze=True, layer="last", layer_idx=None):  # clip-vit-base-patch32
        super().__init__()

        from long_clipdir import longclip



        self.model_transformer, _ = longclip.load("./longclip-L.pt", device=device)
    ...
    def freeze(self):
        self.model_transformer = self.model_transformer.eval()
        #self.train = disabled_train
        for param in self.parameters():
            param.requires_grad = False

    def forward(self, text):
        batch_encoding = self.tokenizer(text).to(self.device)
        # tokens = batch_encoding["input_ids"].to(self.device)
        outputs = self.model_transformer.encode_text_full(batch_encoding)
        # if self.layer == "last":
        #     z = outputs.last_hidden_state
        # elif self.layer == "pooled":
        #     z = outputs.pooler_output[:, None, :]
...
class FrozenOpenRemoteCLIPEmbedder(AbstractEncoder):
    """
    Uses the OpenCLIP transformer encoder for text
    """
    LAYERS = [
    ...
    def __init__(self, arch="ViT-L-14", device=None, max_length=77,
                 freeze=True, layer="last"):
        super().__init__()
        assert layer in self.LAYERS
        self.device = device or ('cuda' if torch.cuda.is_available() else 'mps' if torch.backends.mps.is_available() else 'cpu')
        ...
        ckpt = torch.load(f"{path_to_your_checkpoints}/RemoteCLIP-{arch}.pt", map_location="cpu")
    ...
    def forward(self, text):
        tokens = open_clip.tokenize(text)
        z = self.encode_with_transformer(tokens.to(self.device))
        print('FrozenRemoteCLIPEmbedder')
        return z
